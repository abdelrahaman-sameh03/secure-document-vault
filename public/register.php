...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        try {
            $stmt = db()->prepare('INSERT INTO users (name, email, password_hash, role, two_factor_secret) VALUES (?, ?, ?, ?, ?)');
            $stmt->execute([$name, $email, password_hash($password, PASSWORD_BCRYPT), 'user', totp_generate_secret()]);
            log_system_event('auth.register_success', 'New password account registered.', [
                'id' => (int)db()->lastInsertId(),
                'name' => $name,
                'email' => $email,
            ], ['method' => 'password']);
            set_flash('success', 'Account created successfully. Login now and enable 2FA from Profile.');
            redirect('login.php');
        } catch (PDOException $ex) {
            log_system_event('auth.register_failed', 'Password registration failed.', null, ['email' => $email, 'reason' => 'duplicate_or_database_error']);
...
<section class="auth-wrap">
    <form class="auth-card glass tilt-card reveal" method="post" autocomplete="off">
        <?= csrf_field() ?>
        <p class="eyebrow">Create secure identity</p>
        <h1>Register</h1>
        ...
        <div class="hint">Minimum 8 chars, uppercase, lowercase, number, and special character.</div>
        <button class="btn primary full" type="submit">Create Account</button>

        <div class="oauth-grid">
            <a class="btn oauth-btn full" href="oauth_github.php?flow=register">
                ...
                <span>Sign up with GitHub OAuth</span>
            </a>

            <a class="btn oauth-btn full" href="oauth_google.php?flow=register">
        ...
        <p class="small-center">Already have an account? <a href="login.php">Login</a></p>
    </form>
</section>
